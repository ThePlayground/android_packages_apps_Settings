...
public class UserInterface extends SettingsPreferenceFragment implements
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (mDualPane != null) {
            mDualPane.setChecked(Settings.System.getInt(getActivity().getContentResolver(),Settings.System.DUAL_PANE_SETTINGS, 0) == 1);
        }
	...
	if (preference == mDisableBootAnimation) {
	    ...
	    return true;
            ...
            if (checked) {
                Helpers.getMount("rw");
                new CMDProcessor().su
                        .runWaitFor("mv /system/bin/bugmailer.sh /system/bin/bugmailer.sh.playground");
                Helpers.getMount("ro");
            } else {
                Helpers.getMount("rw");
                new CMDProcessor().su
                        .runWaitFor("mv /system/bin/bugmailer.sh.playground /system/bin/bugmailer.sh");
                Helpers.getMount("ro");
            }
            ...
            boolean checked = ((CheckBoxPreference) preference).isChecked();
            Settings.System.putInt(getActivity().getContentResolver(), Settings.System.DUAL_PANE_SETTINGS, checked ? 1 : 0);
            return true;
        }
